...
import logging
import pysndfile as _pysndfile
from . import util
from .datastructs import SndInfo, SndWriter

from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from .datastructs import SndWriter, sample_t
...
def sndwrite(outfile:str, samples:np.ndarray, sr:int, encoding:str='default',
             ...
             metadata: Dict[str, str]=None, **options) -> None:
    """
...
def sndwrite_chunked(outfile:str, sr: int, encoding: str='auto', fileformat:str=None,
                     metadata: Dict[str, str]=None, **options) -> SndWriter:
    """
...
class _PySndfileWriter(SndWriter):
    ...
    def _open_file(self, channels:int) -> None:
        ...
        self._file = _pysndfile.PySndfile(self.outfile, "w", sndformat, channels, self.sr)
        if self.metadata:
...
class _PySndfile(Backend):
    ...
    def write(self, data:np.ndarray, sr:int, outfile:str, encoding:str) -> None:
        ...
        snd = _pysndfile.PySndfile(outfile, mode='w', format=fmt,
    ...
    def get_sndfile_format(self, fileformat: str, encoding: str) -> int:
        ...
        if fileformat == 'aif':
            fileformat = 'aiff'
        return _pysndfile.construct_format(fileformat, f"{fmt}{bits}")

    def detect_format(self, path:str) -> Opt[str]:
        f = _pysndfile.PySndfile(path)
        return _pysndfile.fileformat_id_to_name.get(f.format())
...
_BACKENDS: Dict[str, Backend] = {
    'pysndfile': _PySndfile(priority=0),
